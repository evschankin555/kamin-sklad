	...
	<?Bitrix\Main\Page\Frame::getInstance()->startDynamicWithID("header-cart");?>
		<?if(!COptimus::IsBasketPage() && !COptimus::IsOrderPage()){?>
			<?if($TEMPLATE_OPTIONS["BASKET"]["CURRENT_VALUE"] == "FLY"):?>
                ...
                $(".basket_fly .fly-basket-scroll").html("");
                $(".podbor_fly").html("");
					...
					$(document).on('click', "#basket_line .basket_fly .opener > div.clicked2", function(){
            $(".basket_sort").html('');                      
						...
						if(window.matchMedia('(min-width: 769px)').matches){  
							...
							if(parseInt($("#basket_line .basket_fly").css("right")) < 0){                   
								$("#basket_line .basket_fly").stop().animate({"right": "0"}, 333, function(){
										...
										$.ajax({
											url: arOptimusOptions['SITE_DIR'] + 'ajax/podbor_fly.php',
											type: 'post',
											success: function(html){
 
												$('#basket_line .basket_fly').addClass('loaded').html(html);
                        return false;
											//	onOpenFlyBasket2(_this);
											}
										});
              ...
              else
              {
                ...
                $(".podbor_fly").html("");
								...
								$("#basket_line .basket_fly").stop().animate({"right": -$("#basket_line .basket_fly").outerWidth()}, 150);
								$("#basket_line .basket_fly .opener > div.clicked2").addClass('small');
