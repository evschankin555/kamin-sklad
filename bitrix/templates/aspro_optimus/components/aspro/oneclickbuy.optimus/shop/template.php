	...
	<div class="form-wr">
		<form method="post" id="one_click_buy_form" action="<?=$arResult['SCRIPT_PATH']?>/script.php">
			<?foreach($arParams['PROPERTIES'] as $field):
				$class.="inputtext";        
         ?>
				<div class="form-control bg <?if($field=="AGREEMENT2") echo 'agree-wrap';?>">  
        
      	<?if($field=="AGREEMENT2"){?> 
            <?

            ?>
    				<label><span>Согласен на обработку  <!--noindex--><a rel="nofollow" target="_blank" onclick="window.open(this.href, '', 'scrollbars=1'); return false;" class="rule1" href="/politika-obrabotki-personalnykh-dannykh.php">персональных данных</a><!--/noindex-->&nbsp;<span class="star">*</span></span></label>
            <input type="checkbox" onclick="proverka1();"   id="chxkbox1" class="one_click_buy_id_<?=$field?>"  name="ONE_CLICK_BUY[<?=$field?>]"  value="Y">
             </div>
      <?
      continue;
      }?>                
...
function proverka1()
{
      ...
      {
            str=str.replace(/id="chxkbox1"/g, 'id="chxkbox1" checked');
      }
