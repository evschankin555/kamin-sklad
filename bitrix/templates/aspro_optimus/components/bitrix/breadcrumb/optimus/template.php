...
if($arResult){
	...
	for($index = 0; $index < $cnt; ++$index){
		...
		if($arItem["LINK"] <> "" && $arItem['LINK'] != GetPagePath() && $arItem['LINK']."index.php" != GetPagePath() || $arSubSections){
			$strReturn .= '<div class="bx-breadcrumb-item'.($arSubSections ? ' drop' : '').'" id="bx_breadcrumb_'.$index.'" itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem">';
			if($arSubSections){
				if($index == ($cnt-1)):
	...
	return '<div class="breadcrumbs" id="navigation" itemscope="" itemtype="https://schema.org/BreadcrumbList">'.$strReturn.'</div>';
}
else{
	return $strReturn;
}
