...
<div class="bx-modal-container-my">
<? if (strlen($arParams["SPAN_ID"])>0) { ?>
<span id="<?=$arParams["SPAN_ID"]?>" style="display: none;">
<? } ?>
...
<? } else { ?>
			<strong class="bx-modal-small-title"><?=GetMessage("CATALOG_SET_CONSTRUCT")?></strong>            
<? } ?>            
		</div>
	</div>
	<div class="row">
		<div class="col-sm-3">
			<div class="bx-original-item-container">
				<?if ($arResult["ELEMENT"]["DETAIL_PICTURE_SMALL"]["src"]):?>
					<img src="<?=$arResult["ELEMENT"]["DETAIL_PICTURE_SMALL"]["src"]?>" style="width: 70px;height: auto;" alt="">
				<?else:?>
			...
			<div class="bx-added-item-table-container">
				<table class="bx-added-item-table">
					...
					<?foreach($arResult["SET_ITEMS"]["DEFAULT"] as $key => $arItem):?>
						<tr
							data-id="<?=$arItem["ID"]?>"
							data-img="<?=$arItem["DETAIL_PICTURE_SMALL"]["src"]?>"
							data-imgb="<?=$arItem["DETAIL_PICTURE"]["src"]?>"
							data-url="<?=$arItem["DETAIL_PAGE_URL"]?>"
							data-name="<?=$arItem["NAME"]?>"
							data-price="<?=$arItem["PRICE_DISCOUNT_VALUE"]?>"
							data-print-price="<?=strip_tags($arItem["PRICE_PRINT_DISCOUNT_VALUE"])?>"
							data-old-price="<?=$arItem["PRICE_VALUE"]?>"
							data-print-old-price="<?=strip_tags($arItem["PRICE_PRINT_VALUE"])?>"
						...
						>
							<td class="bx-added-item-table-cell-img">
								<?if ($arItem["DETAIL_PICTURE_SMALL"]["src"]):?>
									<a href="<?=$arItem["DETAIL_PAGE_URL"]?>"><img src="<?=$arItem["DETAIL_PICTURE_SMALL"]["src"]?>" class="img-responsive img-zoom-my" alt=""></a>
								<?else:?>
		...
		<div class="col-sm-3">
			<div class="bx-constructor-container-result">
				<span class="bx-item-set-current-price" data-role="set-price"><?=$arResult["SET_ITEMS"]["PRICE"]?><span class="price_currency" style="font-size: 22px;"> </span></span>
				<span class="bx-added-item-old-price" data-role="set-old-price"><?
					if ($arResult["SET_ITEMS"]["OLD_PRICE"])
					{
						?><?= $arResult["SET_ITEMS"]["OLD_PRICE"] ?><span class="price_currency"> </span><?
					}
				...
				>
					<div class="bx-catalog-set-item">
						<div class="bx-catalog-set-item-img">
							...
							<?if ($arItem["DETAIL_PICTURE_SMALL"]["src"]):?>
								<a href="<?=$arItem["DETAIL_PICTURE"]["src"]?>" class="fancy"><img src="<?=$arItem["DETAIL_PICTURE_SMALL"]["src"]?>" class="img-responsive img-zoom-my" alt=""/></a>
							<?else:?>
...
$arJsParams = array(
	...
	"mainElementPriceOne" => $arResult["ELEMENT"]["PRICE_ONE_DISCOUNT_VALUE"],
	"mainElementOldPriceOne" => $arResult["ELEMENT"]["PRICE_ONE_VALUE"],
	"mainElementDiffPriceOne" => $arResult["ELEMENT"]["PRICE_ONE_DISCOUNT_DIFFERENCE_VALUE"],
...
<? } else { ?>
<div class="dop__komplekt2" style="display: none;">
<? if (strlen($arParams["SPAN_ID"])>0) { ?>
<span id="<?=$arParams["SPAN_ID"]?>" style="display: none;">
<? } ?>
...
foreach($arResult["SET_ITEMS"]["DEFAULT"] as $key => $arItem):
?>
    <input type="checkbox" name="set_ids" data-qty="<?=$arItem["BASKET_QUANTITY"]?>" data-price="<?=$arItem['PRICE_DISCOUNT_VALUE']?>" value="<?=$arItem["ID"]?>" id="<?=$arParams["SPAN_ID"]?>_dop<?=$i?>"><label for="<?=$arParams["SPAN_ID"]?>_dop<?=$i?>"><?=$arItem["NAME"]?> + <span><?=$arItem["PRICE_DISCOUNT_VALUE"]?> <span class="price_currency"> </span>.</span></label>
<?
$i++;
endforeach;
foreach($arResult["SET_ITEMS"]["OTHER"] as $key => $arItem):
