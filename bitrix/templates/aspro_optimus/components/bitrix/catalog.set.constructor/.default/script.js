...
BX.Catalog.SetConstructor = (function()
{
	var SetConstructor = function(params)
	{
		...
		this.mainElementPriceOne = params.mainElementPriceOne || 0;
		this.mainElementOldPriceOne = params.mainElementOldPriceOne || 0;
		this.mainElementDiffPriceOne = params.mainElementDiffPriceOne || 0;
	...
	SetConstructor.prototype.deleteFromSet = function()
	{
		...
		if (target && target.hasAttribute('data-role') && target.getAttribute('data-role') == 'set-delete-btn')
		{
			...
			itemImgb = item.getAttribute("data-imgb");
													...
													children: [
														BX.create("img", {
															attrs: {
																...
																className: "img-responsive  img-zoom-my"
															}
														})
													]
												})
	...
	SetConstructor.prototype.recountPrice = function()
	{
		...
		{			
			...
			$('.bx-original-item-container .bx-added-item-new-price strong').html(BX.Currency.currencyFormat(this.mainElementPrice, this.currency, true)+' <span class="price_currency"></span>');																	
		}
		else
		{
			sumPrice = this.mainElementPriceOne*this.mainElementBasketQuantity;
			sumOldPrice = this.mainElementOldPriceOne*this.mainElementBasketQuantity;
			sumDiffDiscountPrice = this.mainElementDiffPriceOne*this.mainElementBasketQuantity;		
			$('.bx-original-item-container .bx-added-item-new-price strong').html(BX.Currency.currencyFormat(this.mainElementPriceOne, this.currency, true)+' <span class="price_currency"></span>');														
		}
