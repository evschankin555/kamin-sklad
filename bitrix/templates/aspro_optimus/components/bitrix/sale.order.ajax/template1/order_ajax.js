	...
	BX.Sale.OrderAjaxComponent = {
		...
		initOptions: function()
		{
			...
			if (this.result.TOTAL)
			{
				...
				if (this.result.ISHIDEDISCOUNT) this.options.showPriceWithoutDiscount = false;				
				if (this.result.ISHIDEDISCOUNT) this.options.showDiscountPrice = false;				
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			if (column.id == 'PRICE_FORMATED')
			{
				...
				if (parseFloat(data.DISCOUNT_PRICE) > 0 && !data.ISHIDEDISCOUNT)
				{
