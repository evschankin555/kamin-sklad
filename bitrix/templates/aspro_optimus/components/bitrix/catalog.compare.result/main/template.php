							...
							if ($showRow){?>
								<tr>
									...
									<?foreach($arResult["ITEMS"] as &$arElement){?>
										<td>
<? 
$v=(is_array($arElement["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"])? implode("/ ", $arElement["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"]): $arElement["DISPLAY_PROPERTIES"][$code]["DISPLAY_VALUE"]);
if ($code=="CML2_ARTICLE") {	
	if (strlen($arElement["OFFER_DISPLAY_PROPERTIES"]["ARTICLE"]["DISPLAY_VALUE"])>0) {
		$v=$arElement["OFFER_DISPLAY_PROPERTIES"]["ARTICLE"]["DISPLAY_VALUE"];
	}
}
echo($v);
?>
