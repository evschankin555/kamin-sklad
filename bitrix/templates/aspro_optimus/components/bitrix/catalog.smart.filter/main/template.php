									...
									<?foreach($arItem["VALUES"] as $val => $ar):?>
										...
										<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx_filter_param_label <?=($isSize ? "nab sku" : "");?> <?=($i==$count ? "last" : "");?> <? echo $ar["DISABLED"] ? 'disabled': '' ?>" for="<? echo $ar["CONTROL_ID"] ?>">
											<span class="bx_filter_input_checkbox">
												...
												//$fieldData = CIBlockElement::GetList(array(),array(IBLOCK_ID=>9,CODE=>$ar["URL_ID"]),false,false,array('PREVIEW_PICTURE'));
												$fieldData = CIBlockElement::GetList(array(),array("IBLOCK_ID"=>9,"ID"=>$val),false,false,array('PREVIEW_PICTURE'));

												if($ar_res=$fieldData->GetNext()){
													if ($ar_res['PREVIEW_PICTURE']>0)
													{
												?>
												<span class="bx_filter_btn_color_icon" style="width:124px;height:64px; margin-bottom: 6px;background-size: 80%;background-repeat: no-repeat;background-position: center center;background-image:url('<?=CFile::GetPath($ar_res["PREVIEW_PICTURE"])?>');"></span>												
												<?		
													}
												} 
												}
