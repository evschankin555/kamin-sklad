...
//							strPrice += '</div>';
					...
					//strPrice += '<div class="offers_price" style="margin:0">';
					if (obPrices[j].DISCOUNT_VALUE !== obPrices[j].VALUE && this.showOldPrice)
					{
if($('#ISHIDEDISCOUNT_'+this.product.id).length==0)
{
						strPrice += '<div class="offers_price_old price__old">';
...
if($('#ISHIDEDISCOUNT_'+this.product.id).length==0)
{
						...
						/*strPrice += '<div class="price discount"><div>Выгода</div>';
							strPrice += obPrices[j].PRINT_DISCOUNT_DIFF;							
						strPrice += '</div>';*/

						percent = obPrices[j].DISCOUNT_DIFF_PERCENT;
						strPrice += '<div class="sale_block">';
						strPrice += '<div class="value">' + percent + '%</div>'+
									'<div class="text">Экономия <span>'+obPrices[j].PRINT_DISCOUNT_DIFF+
