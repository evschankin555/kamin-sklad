...
<? } ?>
			...
			?>
			<div class="list_item_wrapp item_wrap item" itemscope itemtype="https://schema.org/Product">
        <meta itemprop="name" content="<?=$name = strip_tags(!empty($arItem['IPROPERTY_VALUES']['ELEMENT_PAGE_TITLE']) ? $arItem['IPROPERTY_VALUES']['ELEMENT_PAGE_TITLE'] : $arItem['NAME'])?>" />
        <meta itemprop="category" content="<?=$arResult['PATH'][0]['NAME']?>" />
        <meta itemprop="description" content="<?=(strlen(strip_tags($arItem['PREVIEW_TEXT'])) ? strip_tags($arItem['PREVIEW_TEXT']) : (strlen(strip_tags($arItem['DETAIL_TEXT'])) ? strip_tags($arItem['DETAIL_TEXT']) : $elementName))?>" />
				<table class="list_item" id="<?=$arItemIDs["strMainID"];?>">
						...
						<div class="image_wrapper_block">
							<div class="stickers">
								...
								<?if($arItem["PRICES"]['BASE']['DISCOUNT_DIFF_PERCENT'] > 0){?>
									<?$percent=$arItem["PRICES"]['BASE']['DISCOUNT_DIFF_PERCENT'];?>
							...
							<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>">
								...
								<?if( !empty($arItem["PREVIEW_PICTURE"]) ):?>
									<img src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>" alt="<?=$a_alt;?>" title="<?=$a_title;?>"  itemprop="image"/>
						...
						<div class="description">
							...
							<div class="wrapp_stockers">
								...
								<div class="brand_block">
									<?if(isset($arItem['DISPLAY_PROPERTIES']['BRAND']) && $arItem['DISPLAY_PROPERTIES']['BRAND']){?>
										<?=$arItem["DISPLAY_PROPERTIES"]['BRAND']['NAME'];?>: <?=$arItem["DISPLAY_PROPERTIES"]['BRAND']['DISPLAY_VALUE'];?>
									<?}?>
								</div>                           
							</div>
							...
							<?if($boolShowProductProps || $boolShowOfferProps):?>
								...
								<div class="show_props dark_link">
									<span class="icons_fa char_title opened"><span><?=GetMessage('PROPERTIES')?></span></span>
								</div>
							<?endif;?>
						</div>
					...
					<td class="information_wrapp" itemprop="offers" itemscope itemtype="https://schema.org/Offer">
						<div class="information">
							...
							<?if($arParams["SHOW_DISCOUNT_TIME"]=="Y"){?>
								<?
								if(!$min_price_id)
									$min_price_id=$_SESSION['CURRENT_LOCATION']['LOCATIONS']['DEFAULT']['PRICES']['PRICE_ID'];								
...
<?}?>                                         
              ...
              <link itemprop="availability" href="https://schema.org/<?=($arItem['CAN_BUY'] ? 'InStock' : 'OutOfStock')?>" />
						</div>
					...
					<tr>
						<td colspan="2"  class="information_wrapp">
						<?if($boolShowProductProps || $boolShowOfferProps):?>
								<div class="props_list_wrapp mob__d" style="display: block;">
									<table class="props_list prod">
										<?if ($boolShowProductProps){
											foreach( $arItem["DISPLAY_PROPERTIES"] as $arProp ){?>
												<?if( !empty( $arProp["VALUE"] ) ){?>
