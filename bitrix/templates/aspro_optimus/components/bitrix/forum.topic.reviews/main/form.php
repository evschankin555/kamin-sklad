	...
	<div style="position:relative; display: block; width:100%;">
		...
		if (strLen($arResult["CAPTCHA_CODE"]) > 0):
			...
			<div class="reviews-reply-field reviews-reply-field-captcha form-control captcha-row clearfix">        
				<label><span><?=GetMessage("F_CAPTCHA_PROMT")?>&nbsp;<span class="star">*</span></span></label>
				<div class="captcha_image">
					<img src="/bitrix/tools/captcha.php?captcha_code=<?=$arResult["CAPTCHA_CODE"]?>" alt="<?=GetMessage("F_CAPTCHA_TITLE")?>" border="0" />
					<input type="hidden" name="captcha_code" value="<?=$arResult["CAPTCHA_CODE"]?>"/>
					<div class="captcha_reload"></div>
				</div>
				<div class="captcha_input">
                <input type="text"  size="30" maxlength="50" class="inputtext captcha" name="captcha_word" tabindex="<?=$tabIndex++;?>" autocomplete="off" />
				</div>				
			</div>
		<?
		endif;
		...
		<?
        /* SMILES */
        if ($arResult["FORUM"]["ALLOW_SMILES"] == "Y" || $arResult["SHOW_SUBSCRIBE"] == "Y"):
        ?>       
		...
		<div class="reviews-reply-buttons">
			<input name="send_button" type="submit" class="button medium" value="<?=GetMessage("OPINIONS_SEND")?>" tabindex="<?=$tabIndex++;?>" <?
			?>onclick="this.form.preview_comment.value = 'N';" />
			<input name="view_button" type="submit" class="button medium transparent" value="<?=GetMessage("OPINIONS_PREVIEW")?>" tabindex="<?=$tabIndex++;?>" <?
