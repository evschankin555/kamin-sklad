...
?>
<div class="reviews-block-container reviews-block-container-articles reviews-reviews-block-container" id="<?=$arParams["FORM_ID"]?>container">
	<div class="reviews-block-outer">
		<div class="reviews-block-inner komment__items">
<?
$iCount = 0;
foreach ($arResult["MESSAGES"] as $res):
	...
	?><table class="reviews-post-table <?=($iCount == 1 ? "reviews-post-first " : "")?><?
		...
		?>" bx-author-id="<?=$res["AUTHOR_ID"]?>" bx-author-name="<?=$res["AUTHOR_NAME"]?>" id="message<?=$res["ID"]?>" itemprop="review" itemscope itemtype="https://schema.org/Review">
		<thead><tr><td>
		<?
...
<div class="item">
		...
		if (intVal($res["AUTHOR_ID"]) > 0 && !empty($res["AUTHOR_URL"])):
			...
			?><span itemprop="author" class="name"><?=$res["AUTHOR_NAME"]?></span><?
		endif;
		?>
		...
		<span class='message-post-date name'><?=$res["POST_DATE"]?></span>
</div>
	...
	<tbody>
	<tr><td>
		<div class="reviews-text item" id="message_text_<?=$res["ID"]?>" itemprop="reviewBody"><?=$res["POST_MESSAGE_TEXT"]?></div>
<?
...
<?			if ($arResult["FORUM"]["ALLOW_QUOTE"] == "Y" && false)
			{
				?>
			...
			if ($arResult["PANELS"]["MODERATE"] == "Y" && false)
			{
				?>
				<span class="separator"></span>
...
<?			}
			if ($arParams["SHOW_RATING"] == "Y" && false)
			{
				?>
