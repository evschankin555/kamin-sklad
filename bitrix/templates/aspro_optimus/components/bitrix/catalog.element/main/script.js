...
$(document).ready(function (){
    $('.dop__komplekt2 input').on('change', function(){
        var old_price = parseInt($('.prices_block .price1').attr('data-price'));
        var old_price2 = parseInt($('.prices_block .price2').attr('data-price'));
        var dop_price = parseInt($(this).attr('data-price'));
        
        if($(this).prop('checked')) var new_price = old_price+dop_price;
        else var new_price = old_price-dop_price;
		
        if($(this).prop('checked')) var new_price2 = old_price2+dop_price;
        else var new_price2 = old_price2-dop_price;
        
        if ($('.prices_block .price1').length>0) 
		{
			$('.prices_block .price1 .price_value').html(number_format(new_price, 0, '', ' '));
			$('.prices_block .price1').attr('data-price', new_price);
		}
        if ($('.prices_block .price2').length>0) 
		{
			$('.prices_block .price2 .price_value').html(number_format(new_price2, 0, '', ' '));
			$('.prices_block .price2').attr('data-price', new_price2);
		}
    })
...
window.JCCatalogElement.prototype.SetSliderPict = function(obj, slider, config)
{
	...
	if(countPhoto>1){
		...
		container.find('.thumbs').delegate('li:not(.current):not(.flex-nav-prev):not(.flex-nav-next)', 'click', function(e){
			var srcSmall='',
				srcBig='',
				width='',
				height=''
...
{
	$(".info_item .top_info .dop__komplekt input[type='checkbox']").prop("checked", false);
	var prices = '';
...
//							strPrice += '</div>';
					...
					strPrice += '<div class="price price1" data-price="'+obPrices[j].DISCOUNT_VALUE+'">';
...
if ($('#ISHIDEDISCOUNT').length==0) {
						strPrice += '<div class="vigod">';

						strPrice += '<div class="price discount price2" data-price="'+obPrices[j].VALUE+'">';
							strPrice += obPrices[j].PRINT_VALUE;
						strPrice += '</div>';						

							strPrice += '<br>Выгода: ';
							strPrice += obPrices[j].PRINT_DISCOUNT_DIFF;
