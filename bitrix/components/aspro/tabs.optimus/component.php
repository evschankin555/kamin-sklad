...
if($arShowProp){
	foreach($arShowProp as $key=>$prop){
		$arItems=array();
// Создаем объект класса COptimusCache для вызова метода CIBlockElement_GetList()
        $optimusCache = new COptimusCache();

// Вызываем метод CIBlockElement_GetList() через объект $optimusCache
        $arItems = $optimusCache->CIBlockElement_GetList(
            array(
                'CACHE' => array(
                    "MULTI" => "N",
                    "TAG" => $optimusCache->GetIBlockCacheTag($arParams["IBLOCK_ID"])
                )
            ),
            array_merge(
                $arFilter,
                $arrFilter,
                array(
                    "PROPERTY_" . $arParams["TABS_CODE"] . "_VALUE" => array($prop)
                )
            ),
            false,
            array("nTopCount" => 1),
