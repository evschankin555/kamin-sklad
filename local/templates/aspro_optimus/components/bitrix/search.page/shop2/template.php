...
<?if(isset($arResult["REQUEST"]["ORIGINAL_QUERY"])):
	...
	<?elseif(count($arResult["SEARCH"])>0):?>
		...
		<?foreach($arResult["SEARCH"] as $arItem):
			$phrase_id=$arResult["NAV_RESULT"]->Statistic->phrase_id;
			if (strpos($arItem["URL"],"#YEAR#")!==false)
			{
				$arItem["URL"]=$arItem["URL_WO_PARAMS"]."?sphrase_id=".$phrase_id;
				$arItem["URL"]=str_replace("#YEAR#",strtok($arItem["DATE_FROM"], '-'),$arItem["URL"]);
			}
		?>
