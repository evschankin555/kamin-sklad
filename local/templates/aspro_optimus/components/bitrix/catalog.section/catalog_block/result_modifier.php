			...
			if ('TYPE_1' == $arParams['TYPE_SKU'] && $arParams['DISPLAY_TYPE'] !='table')
			{
				...
				foreach ($arItem['OFFERS'] as $keyOffer => $arOffer)
				{
					...
					if($arOffer['PRICES'])
					{
						...
						{
							if($arItem['CATALOG_GROUP_NAME_'.$arOfferPrice['PRICE_ID']])
								$arOffer['PRICES'][$priceKey]['GROUP_NAME'] = $arItem['CATALOG_GROUP_NAME_'.$arOfferPrice['PRICE_ID']];
						}
					}
...
foreach ($arResult['ITEMS'] as $k=>$arItem){
   $arResult['ITEMS'][$k]['DETAIL_PAGE_URL'] = $arRes[$arResult['ITEMS'][$k]['ID']];
	...
	foreach($arDiscounts3 as $key => $val){
		foreach($val as $key2 => $val2){		
			if (strpos($val2["NAME"],"HIDE")!==false) { $isHideDiscount=true; break; }
			if ($val2["LAST_DISCOUNT"]=="Y") $isBreak=true;
			if ($isBreak) break;
		}
		if ($isBreak) break;
	}
	$arResult['ITEMS'][$k]['ISHIDEDISCOUNT'] = $isHideDiscount;
