...
{
	$productSlider[]=array(
				"ID" => $arResult['DISPLAY_PROPERTIES']["DRAWING"]["FILE_VALUE"]["ID"],
				"SRC" => $arResult['DISPLAY_PROPERTIES']["DRAWING"]["FILE_VALUE"]["SRC"],
				"WIDTH" => $arResult['DISPLAY_PROPERTIES']["DRAWING"]["FILE_VALUE"]["WIDTH"],
				"HEIGHT" => $arResult['DISPLAY_PROPERTIES']["DRAWING"]["FILE_VALUE"]["HEIGHT"],
				"ALT" => $arResult['DISPLAY_PROPERTIES']["DRAWING"]["FILE_VALUE"]["DESCRIPTION"],
				"TITLE" => $arResult['DISPLAY_PROPERTIES']["DRAWING"]["FILE_VALUE"]["DESCRIPTION"]
	);
}
	...
	foreach($productSlider as $i => $arImage){
		$productSlider[$i] = array_merge(
			$arImage, array(
				...
				"SMALL" => CFile::ResizeImageGet($arImage["ID"], array("width" => 340, "height" => 340), BX_RESIZE_IMAGE_PROPORTIONAL, true, array(),false,95),
				"THUMB" => CFile::ResizeImageGet($arImage["ID"], array("width" => 50, "height" => 50), BX_RESIZE_IMAGE_PROPORTIONAL, true, array(),false,95),			
			)
		);
	}
}
...
$db_res = CIBlockElement::GetList(Array("SORT"=>"ASC"), Array("IBLOCK_ID"=>19,array("LOGIC" => "OR",array("PROPERTY_ELEMENTS"=>$arResult["ID"]),array("PROPERTY_SECTIONS"=>$arResult["IBLOCK_SECTION_ID"]))), false, false, array("NAME","PROPERTY_VIDEO"));
while ($res = $db_res->GetNext()) {
	$arResult["VIDEOS"][]='<iframe style="border:0;width:917px;height:516px" src="//www.youtube.com/embed/'.$res["PROPERTY_VIDEO_VALUE"].'" ></iframe>';
}
...
{        		
	...
	$cp->arResult['OFFER_GROUP'] = $arResult['OFFER_GROUP'];
	$cp->arResult['OFFERS_IBLOCK'] = $arResult['OFFERS_IBLOCK'];
	$cp->SetResultCacheKeys(array('OFFERS','OFFER_GROUP','OFFERS_IBLOCK','PROPERTIES'));
}	
...
$arDiscounts2 = CCatalogDiscount::GetDiscountByProduct(
	$arResult["ID"],
...
$arDiscounts3=array();
foreach($arDiscounts2 as $key => $val){
...
foreach($arDiscounts3 as $key => $val){
	foreach($val as $k => $v){		
		if (strpos($v["NAME"],"HIDE")!==false) { $isHideDiscount=true; break; }
		if ($v["LAST_DISCOUNT"]=="Y") $isBreak=true;
		if ($isBreak) break;
	}
	if ($isBreak) break;
}
$arResult['ISHIDEDISCOUNT'] = $isHideDiscount;   
